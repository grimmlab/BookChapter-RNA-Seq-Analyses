...
1. ##### **Download the data and perform quality control (download_RNASeq_reads_and_QC.sh)** 
   ...
   *Output:* The `fastq-dump` downloads the following files into the **reads/rawreads** folder:
...
2. ##### **Perform reference alignment using genome sequence (reference_based_alignment.sh)**
   ...
   *Output:* The `STAR`  genome indexing will generated indexed genome files into the **data/STAR_Genome_Index** folder: 
   ...
   *Output:* The `STAR`  genome alignment will generated in files into the **analysis/mapping/star** folder: 
...
*Output:* The `bowtie-build`  genome indexing will generated indexed genome files into the **data/TOPHAT_Genome_Index** folder: 

```bash
   ~/RNASeq_project/data/TOPHAT_Genome_Index$ls -1 GRCm38.p6.genome.1.bt2
...
The outputs will be generated in **analysis/quantification/bedtools-count** folder.  In the end all the  bedtools generated output files are merged to created final file **Read_per_features_combined.csv** in same folder.
...
The outputs will be generated in **analysis/quantification/htseq-count folder**. In the end all the  HTSeq generated output files are combined to created final file **Read_per_features_combined.csv** in same folder.
...
6. **Differential Expression (differential_expression.sh)**
   ...
   *Note! For more details about differential expression read seection XXX in the book and for analysis details check* *[DESeq2](https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html).* 
   ...
   *Description:*
   ...
   Differentially expressed outfiles for bedtools and HTSeq are generated in **analysis/DE/dexseq/**

   *Note! For more details about differential expression read seection XXX in the book and for analysis details check* *[DEXSeq](https://bioconductor.org/packages/devel/bioc/vignettes/DEXSeq/inst/doc/DEXSeq.html)*
