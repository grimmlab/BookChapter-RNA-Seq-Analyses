...
p$add_argument("--genecount", help="Enter the htseq generated gene count file")
...
countTable <- round(rawCountTable[,1:length(names(rawCountTable))])
...
# Plot dispersions
pdf(paste(outputFolder,"qc-dispersions_htseq.pdf",sep=""))#, w=1000, h=1000, pointsize=20)
...
pdf(paste(outputFolder,"qc-heatmap-samples_htseq.pdf",sep=""))#, w=1000, h=1000, pointsize=20)
...
pdf(paste(outputFolder,"PCA-samples_htseq.pdf",sep=""))#, w=1000, h=1000, pointsize=20)
rld <- rlog(dds)
...
head(resdata)
## Write results
write.csv(resdata, file=paste(outputFolder,comp,"_diffexpr-results_htseq.csv",sep=""))

## Examine plot of p-values
#hist(res$pvalue, breaks=50, col="grey")

## Examine plot of p-values
pdf(paste(outputFolder,"pvalue_histogram_50_htseq.pdf", sep=""))#, 1500, 1000, pointsize=20)
hist(res$pvalue, breaks=50, col="grey")
dev.off()
...
pdf(paste(outputFolder,"diffexpr-maplot_htseq.pdf", sep=""))#, 1500, 1000, pointsize=20)
